        ...
        return function(...)
            ...
            while true do
                ...
                elseif op == 3 or op == 96 then
                    print("CALL_NO_RESULT", nil, ins[2], ins[2], ins[3])
                    local v112 = ins[2]
                    memory[v112](table_unpack(memory, v112 + 1, ins[3]))  -- 调用函数并传递参数
                elseif op == 10 or op == 18 then
                    print("TAILCALL", ins[2], ins[2]+1, top_index)
                    local v178 = ins[2]
                    return memory[v178](table_unpack(memory, v178 + 1, top_index))  -- 调用函数并返回
                elseif op == 12 or op == 49 then
                    print("TAILCALL", ins[2], ins[2]+1, ins[3])
                    local v114 = ins[2]
                    return memory[v114](table_unpack(memory, v114 + 1, ins[3]))  -- 调用函数并返回
                elseif op == 4 or op == 14 then
                    print("SETTABLE", ins[2], ins[3], ins[4])
                    local v164 = ins[2]
                    local v165 = memory[ins[3]]
                    memory[v164 + 1] = v165
                    memory[v164] = v165[ins[4]]  -- 表索引赋值
                elseif op == 5 or op == 50 then
                    print("MOVE", ins[2], ins[3])
                    memory[ins[2]] = memory[ins[3]]  -- 内存复制
                elseif op == 6 or op == 90 then
                    print("RETURN0")
                    return  -- 退出循环
                elseif op == 7 or op == 17 then
                    print("SETTABLE1", ins[2], ins[3], ins[4])
                    memory[ins[2]][ins[3]] = memory[ins[4]]  -- 表字段赋值
                elseif op == 9 or op == 70 then
                    print("LEN", ins[2], ins[3])
                    memory[ins[2]] = #memory[ins[3]]  -- 获取表长度
                elseif op == 11 or op == 58 then
                    print("OP_LOADENV", ins[2], ins[3])
                    if ins[3] == "_ENV" then
                        memory[ins[2]] = funcenv  -- 设置环境变量
                    else
                        memory[ins[2]] = funcenv[ins[3]]  -- 从环境中获取值
                    end

                elseif op == 13 or op == 68 then
                    local REG_A = ins[2]
                    local v183, v184 = table_pack(memory[REG_A](table_unpack(memory, REG_A + 1, top_index)))
                ...
                elseif op == 30 or op == 60 then
                    local REG_A = ins[2]
                    local v205, v206 = table_pack(memory[REG_A](table_unpack(memory, REG_A + 1, ins[3])))
                ...
                elseif op == 67 or op == 91 then
                    local v245 = ins[2]
                    local v246 = { memory[v245](table_unpack(memory, v245 + 1, ins[3])) }  -- 调用函数并存储结果
                    local v247 = 0
                ...
                elseif op == 19 or op == 69 then
                    memory[ins[2]] = memory[ins[3]][memory[ins[4]]]  -- 表索引取值
                ...
                elseif op == 22 or op == 56 then
                    memory[ins[2]] = funcupvals[ins[3]]  -- 从 v62 获取值
                elseif op == 23 or op == 25 then
                    print("GETTABLE", ins[2], ins[3], ins[4])
                    memory[ins[2]] = memory[ins[3]][ins[4]]  -- 表字段取值

                elseif op == 26 or op == 45 then
                    local v197 = ins[2]
                    memory[v197] = memory[v197]()  -- 调用函数并存储结果
                elseif op == 27 or op == 74 then
                    if memory[ins[2]] <= ins[4] then
                        pc = pc + 1
                    else
                        pc = ins[3]  -- 条件跳转
                    end
                elseif op == 28 or op == 101 then
                    print("NEWFUNC", ins[2], ins[3], ins[4])
                    local v122 = subs[ins[3]]
                    local real_upval = {}
                    ...
                    memory[ins[2]] = v29(v122, upvals, funcenv)  -- 创建闭包
                ...
                elseif op == 43 or op == 82 then
                    local v214 = ins[2]
                    memory[v214] = memory[v214](table_unpack(memory, v214 + 1, top_index))  -- 调用函数并存储结果
                elseif op == 46 or op == 54 then
                    funcenv[ins[3]] = memory[ins[2]]  -- 设置环境变量
                elseif op == 47 or op == 89 then
                    memory[ins[2]] = v29(subs[ins[3]], nil, funcenv)  -- 创建闭包

              
                elseif op == 51 or op == 65 then
                    memory[ins[2]] = ins[3]  -- 赋值常量
             
                elseif op == 57 or op == 64 then
                    memory[ins[2]][memory[ins[3]]] = memory[ins[4]]  -- 表字段赋值
            
                elseif op == 61 or op == 88 then
                    local v233 = ins[3]
                    local v235 = memory[v233]
                    for v412 = v233 + 1, ins[4] do
                ...
                elseif op == 66 or op == 84 then
                    local v138 = ins[2]
                    memory[v138] = memory[v138](table_unpack(memory, v138 + 1, ins[3]))  -- 调用函数并存储结果

                elseif op == 71 or op == 100 then
                    local v250 = ins[2]
                    local v251 = memory[v250]
                    for v413 = v250 + 1, top_index do
                        table_insert(v251, memory[v413])  -- 表插入
                    end
                elseif op == 53 then
                    local v227 = ins[2]
                    local v228 = memory[v227]
                    for v307 = v227 + 1, ins[3] do
                        table_insert(v228, memory[v307])  -- 表插入
                    end
                elseif op == 72 or op == 86 then
                    print("CALL_NO_RESULT_NO_PARAM", ins[2])
                    memory[ins[2]]()  -- 调用函数
                elseif op == 76 or op == 99 then
                    memory[ins[2]] = memory[ins[3]] + ins[4]  -- 加法赋值
                ...
                elseif op == 78 or op == 94 then
                    local v261 = ins[2]
                    return table_unpack(memory, v261, top_index)  -- 返回解包值
                elseif op == 33 then
                    local v209 = ins[2]
                    return table_unpack(memory, v209, v209 + ins[3])  -- 返回解包值
