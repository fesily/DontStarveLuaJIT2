...
public:
    ...
    // Preferred: schema auto-deduced from Fn.
    template <class Fn>
    bool register_service(const char *name, Fn fn) {
        using Sig = ds::plugin::GiSignature<Fn>;
        return register_service(name, Sig::types, Sig::ntypes, reinterpret_cast<void *>(fn));
    }
    // GameInjector Lua export (module_init window). Preferred: auto-deduce from Fn.
    ...
    template <class Fn>
    bool register_game_injector_export(const char *name, Fn fn) {
        using Sig = ds::plugin::GiSignature<Fn>;
        return register_game_injector_export(name, Sig::types, Sig::ntypes, reinterpret_cast<void *>(fn));
    }
